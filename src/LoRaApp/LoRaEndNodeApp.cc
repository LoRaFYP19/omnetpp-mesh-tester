...
Define_Module (LoRaEndNodeApp);

void LoRaEndNodeApp::initialize(int stage) {
...
//        }
        ...
        if (numberOfDestinationsPerNode == 0 ) {
            numberOfDestinationsPerNode = numberOfEndNodes-1;
...
std::pair<double, double> LoRaEndNodeApp::generateUniformCircleCoordinates(
...
void LoRaEndNodeApp::handleMessage(cMessage *msg) {
...
void LoRaEndNodeApp::handleSelfMessage(cMessage *msg) {
    ...
    if (lrmc->fsm.getState() == IDLE ) {
        ...
        if (!sendPacketsContinuously && routingPacketsDue) {
            ...
            for (int i=0; i<numberOfEndNodes; i++) {
                LoRaEndNodeApp *lrndpp = (LoRaEndNodeApp *) getParentModule()->getParentModule()->getSubmodule("loRaNodes", i)->getSubmodule("LoRaEndNodeApp");
...
void LoRaEndNodeApp::handleMessageFromLowerLayer(cMessage *msg) {
    receivedPackets++;
...
void LoRaEndNodeApp::manageReceivedRoutingPacket(cMessage *msg) {
...
void LoRaEndNodeApp::manageReceivedPacketToForward(cMessage *msg) {
...
void LoRaEndNodeApp::manageReceivedAckPacketToForward(cMessage *msg) {
    receivedAckPackets++;
    receivedAckPacketsToForward++;
}

void LoRaEndNodeApp::manageReceivedDataPacketToForward(cMessage *msg) {
    receivedDataPackets++;
...
void LoRaEndNodeApp::manageReceivedPacketForMe(cMessage *msg) {
...
void LoRaEndNodeApp::manageReceivedDataPacketForMe(cMessage *msg) {
    receivedDataPackets++;
...
void LoRaEndNodeApp::manageReceivedAckPacketForMe(cMessage *msg) {
    receivedAckPackets++;
...
bool LoRaEndNodeApp::handleOperationStage(LifecycleOperation *operation, int stage,
...
simtime_t LoRaEndNodeApp::sendDataPacket() {
...
simtime_t LoRaEndNodeApp::sendForwardPacket() {
...
simtime_t LoRaEndNodeApp::sendRoutingPacket() {
...
void LoRaEndNodeApp::generateDataPackets() {
    ...
    if (!onlyNode0SendsPackets || nodeId == 0) {
        ...
        while (destinations.size() < numberOfDestinationsPerNode
                && numberOfEndNodes - 1 - destinations.size() > 0) {

            int destination = intuniform(0, numberOfEndNodes - 1);
...
void LoRaEndNodeApp::increaseSFIfPossible() {
    if (loRaSF < 12) {
        // char text[32];
...
bool LoRaEndNodeApp::isNeighbour(int neighbourId) {
...
bool LoRaEndNodeApp::isRouteInSingleMetricRoutingTable(int id, int via) {
...
int LoRaEndNodeApp::getRouteIndexInSingleMetricRoutingTable(int id, int via) {
    int singleMetricRoutesCount = end(singleMetricRoutingTable) - begin(singleMetricRoutingTable);
...
bool LoRaEndNodeApp::isRouteInDualMetricRoutingTable(int id, int via, int sf) {
...
int LoRaEndNodeApp::getRouteIndexInDualMetricRoutingTable(int id, int via, int sf) {
    int dualMetricRoutesCount = end(dualMetricRoutingTable) - begin(dualMetricRoutingTable);
...
bool LoRaEndNodeApp::isKnownNode(int knownNodeId) {
...
bool LoRaEndNodeApp::isACKed(int nodeId) {
...
bool LoRaEndNodeApp::isPacketForwarded(cMessage *msg) {
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *>(msg);
...
bool LoRaEndNodeApp::isPacketToBeForwarded(cMessage *msg) {
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *>(msg);
...
bool LoRaEndNodeApp::isDataPacketForMeUnique(cMessage *msg) {
    LoRaAppPacket *packet = check_and_cast<LoRaAppPacket *>(msg);
...
int LoRaEndNodeApp::getBestRouteIndexTo(int destination) {
...
void LoRaEndNodeApp::sanitizeRoutingTable() {
...
int LoRaEndNodeApp::getSFTo(int destination) {
...
simtime_t LoRaEndNodeApp::calculateTransmissionDuration(cMessage *msg) {
...
simtime_t LoRaEndNodeApp::getTimeToNextRoutingPacket() {
...
simtime_t LoRaEndNodeApp::getTimeToNextDataPacket() {
...
simtime_t LoRaEndNodeApp::getTimeToNextForwardPacket() {
